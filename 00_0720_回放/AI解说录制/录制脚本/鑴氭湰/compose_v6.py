...
SRC_USE=439.0; FSPTS=0.83   # settlement rec t=420 -> video ~360 (voice "送达" at 357)
...
fc=[f"[0:v]scale={W}:2020,setsar=1,fps=30,fade=t=out:st=10.0:d=0.5[v0]",
    f"[1:v]trim=0:439,setpts={FSPTS}*PTS,scale={W}:2020,setsar=1,fps=30[v1]",
    "[v0][v1]concat=n=2:v=1[cat0]","[cat0]tpad=stop_mode=clone:stop_duration=16[cat]",
...
args+=["-filter_complex",";".join(fc),"-map",f"[{prev}]","-map","[aout]","-t",str(AUDIO_DUR),
       ...
       "-c:a","aac","-b:a","192k","-movflags","+faststart",f"{SCR}/rec/42177_v6.mp4","-loglevel","error","-stats"]
print("encoding v6 @4K ...",flush=True)
